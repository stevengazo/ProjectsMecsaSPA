...
namespace ProjectsMecsaSPA.Services
{
    public class FileStorageService
    {
        ...
        // Genera el directorio de una factura dentro del proyecto
        public async Task GenerateFilesDirectoryAsync(int projectId)
        {
            try
            {
                Directory.CreateDirectory(Path.Combine(_baseRoute, projectId.ToString(), "Files"));
                var billDirectory = Path.Combine(_baseRoute, projectId.ToString(), "Files");
                if (!Directory.Exists(billDirectory))
                {
                    Directory.CreateDirectory(billDirectory);
                }
        ...
        public async Task<string> UploadFilesAsync(int projectId, IBrowserFile file)
        {
            // Generar directorios si no existen
            await GenerateProjectDirectoryAsync(projectId);
            await GenerateFilesDirectoryAsync(projectId);
            ...
            var relativePath = Path.Combine("projectsdata", projectId.ToString(), "Files", fileName);
